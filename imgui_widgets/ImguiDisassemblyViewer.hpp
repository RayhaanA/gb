...
struct DisassemblyViewer {
  ...
  void drawWindow(CPU& cpu) {
    ...
    int32_t max = std::min(static_cast<int32_t>(disassemblyList.size()),
                           indexMap[cpu.getPC()] + 10000);
    ...
    if (ImGui::BeginMenuBar()) {
      if (ImGui::BeginMenu("Options")) {
        ...
        ImGui::MenuItem("Show instruction jump window", nullptr,
                        &showInstructionJumpWindow);
        ...
        if (clicked & 1 && targetPC >= 0 && targetPC < cpu.getMemory().size()) {
          ...
          while (cpu.getPC() != static_cast<uint16_t>(targetPC)) {
            if (cpu.getPC() == cpu.getPCPrev()) {
              std::cerr << "Reached end of program before reaching target!"
                        << std::endl;
              break;
            }
            cpu.tick();
    ...
    for (int32_t i = 0; i < disassemblyList.size(); ++i) {
      if (i == indexMap[cpu.getPC()] && trackCurrentInstruction) {
        ImGui::TextColored(ImVec4(0.03f, 0.94f, 0.0f, 1.0f),
                           disassemblyList[i].c_str());
