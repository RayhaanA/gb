...
// to a mono-space font before caling this.
//
// Usage:
...
//   // create a window and draw memory editor (if you already have a window,
//   use DrawContents())
//
// Usage:
...
// - v0.11: always refresh active text input with the latest byte from source
// memory if it's not being edited.
// - v0.12: added OptMidRowsCount to allow extra spacing every XX rows.
// - v0.13: added optional ReadFn/WriteFn handlers to access memory via a
// function. various warning fixes for 64-bits.
// - v0.14: added GotoAddr member, added GotoAddrAndHighlight() and
// highlighting. fixed minor scrollbar glitch when resizing.
// - v0.15: added maximum window width. minor optimization.
// - v0.16: added OptGreyOutZeroes option. various sizing fixes when resizing
// using the "Rows" drag.
// - v0.17: added HighlightFn handler for optional non-contiguous highlighting.
// - v0.18: fixes for displaying 64-bits addresses, fixed mouse click gaps
// introduced in recent changes, cursor tracking scrolling fixes.
// - v0.19: fixed auto-focus of next byte leaving WantCaptureKeyboard=false for
// one frame. we now capture the keyboard during that transition.
// - v0.20: added options menu. added OptShowAscii checkbox. added optional
// HexII display. split Draw() in DrawWindow()/DrawContents(). fixing glyph
// width. refactoring/cleaning code.
// - v0.21: fixes for using DrawContents() in our own window. fixed HexII to
// actually be useful and not on the wrong side.
// - v0.22: clicking Ascii view select the byte in the Hex view. Ascii view
// highlight selection.
// - v0.23: fixed right-arrow triggering a byte write.
// - v0.24: changed DragInt("Rows" to use a %d data format (which is desirable
// since imgui 1.61).
...
// - v0.31: added OptUpperCaseHex option to select lower/upper casing display
// [@samhocevar]
// - v0.32: changed signatures to use void* instead of unsigned char*
// - v0.33: added OptShowOptions option to hide all the interactive option
// setting.
...
struct MemoryEditor {
  ...
  void drawWindow(const char* title, void* mem_data, size_t mem_size,
    ...
    if (ImGui::Begin(title, &Open,
      ...
      ImGui::SetNextWindowSize(ImVec2(Display::MEMORY_VIEWER_WINDOW_W,
                                      Display::MEMORY_VIEWER_WINDOW_H));
      ImGui::SetNextWindowPos(ImVec2(Display::MEMORY_VIEWER_WINDOW_X,
                                     Display::MEMORY_VIEWER_WINDOW_Y));
  ...
  void DrawContents(void* mem_data_void, size_t mem_size,
    ...
    if (data_editing_addr_next != (size_t)-1 &&
        (data_editing_addr_next / Cols) != (data_editing_addr_backup / Cols)) {
      // Track cursor movements
      const int scroll_offset = ((int)(data_editing_addr_next / Cols) -
                                 (int)(data_editing_addr_backup / Cols));
      const bool scroll_desired =
          (scroll_offset < 0 &&
           data_editing_addr_next < visible_start_addr + Cols * 2) ||
          (scroll_offset > 0 &&
           data_editing_addr_next > visible_end_addr - Cols * 2);
      if (scroll_desired)
      ...
      for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
        ...
        if (DataEditingAddr == addr) {
          ...
          struct UserData {
            ...
            // such a ugly mess we may be better off not using InputText() at
            // all here.
            static int Callback(ImGuiInputTextCallbackData* data) {
              ...
              if (data->SelectionStart == 0 &&
                ...
                // When not editing a byte, always rewrite its content (this is
                // a bit tricky, since InputText technically "owns" the master
                // copy of the buffer we edit it in there)
          ...
          if (ImGui::InputText("##data", DataInputBuf, 32, flags,
      ...
      if (OptShowAscii) {
        ...
        for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
          ...
          draw_list->AddText(pos,
                             (display_c == '.') ? color_disabled : color_text,
  ...
  void DrawOptionsLine(const Sizes& s, void* mem_data, size_t mem_size,
    ...
    ImGui::SameLine();
    ImGui::PushItemWidth((s.AddrDigitsCount + 1) * s.GlyphWidth +
  ...
  void DrawPreviewLine(const Sizes& s, void* mem_data_void, size_t mem_size,
    ...
    ImGui::SameLine();
    ImGui::PushItemWidth((s.GlyphWidth * 10.0f) + style.FramePadding.x * 2.0f +
    ...
    ImGui::PopItemWidth();
    ImGui::SameLine();
    ImGui::PushItemWidth((s.GlyphWidth * 6.0f) + style.FramePadding.x * 2.0f +
                         style.ItemInnerSpacing.x);
