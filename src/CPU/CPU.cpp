...
void CPU::tick() {
  ...
  if (Globals::IME && !halted) {
    ...
    // Halt mode needs an extra cycle to exit
    incrementCycleCount();
  }

  if (!interruptHandledThisTick) {
    uint16_t op = readMemoryAndIncrementCycles(PC.data);
    ...
    if (op == 0xCB) {
      op = readMemoryAndIncrementCycles(PC++.data);
      currInstruction <<= 8;
      currInstruction |= op;
...
void CPU::incrementCycleCount() {
  cycles += Globals::CYCLES_PER_INCREMENT;
  ...
  if (timers->getTIMAInterruptRequest()) {
    ...
    memory->directWrite(memory->directRead(Address::TMA_REG_ADDR),
                        Address::TIMA_REG_ADDR);
  ...
  if (cycles > Globals::FREQUENCY) {
    cycles -= Globals::FREQUENCY;
  }
  ...
  frameCycles += Globals::CYCLES_PER_INCREMENT;
  if (frameCycles > Globals::CYCLES_PER_FRAME) {
    frameDone = true;
    frameCycles -= Globals::CYCLES_PER_FRAME;
  }
}
...
void CPU::handleInterrupts() {
  ...
  uint8_t IF = memory->directRead(Address::IF_REG_ADDR);
  uint8_t IE = memory->directRead(Address::IE_REG_ADDR);
  ...
  memory->directWrite((IF & ~(1 << activeInterrupt)), Address::IF_REG_ADDR);
...
void CPU::setFlags(uint8_t value) { F.data |= value; }

void CPU::unsetFlags(uint8_t value) { F.data &= ~value; }

bool CPU::checkFlags(uint8_t value) { return F.data & value; }

void CPU::requestInterrupt(uint8_t data) {
  memory->getMemory()[Address::IF_REG_ADDR] |= data & 0x1F;
}

void CPU::clearInterruptRequest(uint8_t data) {
  memory->getMemory()[Address::IF_REG_ADDR] &= ~data;
}

bool CPU::checkForInterrupts() {
  bool interrupt = memory->directRead(Address::IF_REG_ADDR) &
                   memory->directRead(Address::IE_REG_ADDR) & 0x1F;
  return interrupt;
}
...
void CPU::triggerOAMBug(bool read) {
  ...
  if (!read) {
    ...
    uint8_t newValue = oamBugWrite(memory->directRead(rowStartAddr),
                                   memory->directRead(rowStartAddr - 8),
                                   memory->directRead(rowStartAddr - 4));
    memory->directWrite(newValue, rowStartAddr);

    newValue = oamBugWrite(memory->directRead(rowStartAddr + 1),
                           memory->directRead(rowStartAddr - 7),
    ...
    uint8_t newValue = oamBugRead(memory->directRead(rowStartAddr),
                                  memory->directRead(rowStartAddr - 8),
                                  memory->directRead(rowStartAddr - 4));
    memory->directWrite(newValue, rowStartAddr);

    newValue = oamBugRead(memory->directRead(rowStartAddr + 1),
                          memory->directRead(rowStartAddr - 7),
                          memory->directRead(rowStartAddr - 3));
    memory->directWrite(newValue, rowStartAddr + 1);
...
void CPU::triggerOAMBugIncrease() {
  ...
  if (!(ppu->getCurrSpriteIndex() < 8 ||
    ...
    newValue = oamBugReadIncrease(memory->directRead(rowStartAddr - 15),
                                  memory->directRead(rowStartAddr - 7),
                                  memory->directRead(rowStartAddr + 1),
                                  memory->directRead(rowStartAddr - 3));
...
void CPU::dmaCopyByte() {
  if (numBytesCopiedDuringDMA < 159) {
    memory->directWrite(
        memory->directRead(dmaSourceAddress + numBytesCopiedDuringDMA),
        Address::OAM_TABLE_ADDR + numBytesCopiedDuringDMA);
...
void CPU::callInstruction(uint16_t instruction) {
  switch (instruction) {
    ...
    case 0xBF:
      ...
      break;
    case 0xC0:
      RET(!checkFlags(Z_FLAG));
      break;
    case 0xC1:
      POP(BC);
      break;
    case 0xC2:
      JP(!checkFlags(Z_FLAG), readU16());
      break;
    case 0xC3:
      JP(readU16());
      break;
    case 0xC4:
      CALL(!checkFlags(Z_FLAG), readU16());
      break;
    case 0xC5:
      PUSH(BC);
      break;
    ...
    case 0xCF:
      ...
      break;
    case 0xD0:
      RET(!checkFlags(C_FLAG));
      break;
    case 0xD1:
      POP(DE);
      break;
    case 0xD2:
      JP(!checkFlags(C_FLAG), readU16());
      break;
    case 0xD4:
      CALL(!checkFlags(C_FLAG), readU16());
      break;
    case 0xD5:
      PUSH(DE);
      break;
    ...
    case 0xDF:
      ...
      break;
    case 0xE0:
      LD(static_cast<uint16_t>(0xFF00u + readU8()), A);
      break;
    case 0xE1:
      POP(HL);
      break;
    case 0xE2:
      LD(static_cast<uint16_t>(0xFF00u + C.data), A);
      break;
    case 0xE5:
      PUSH(HL);
      break;
    ...
    case 0xEF:
      ...
      break;
    case 0xF0:
      LD(A, static_cast<uint16_t>(0xFF00u + readU8()));
      break;
    case 0xF1:
      POP(AF);
      break;
    case 0xF2:
      LD(A, static_cast<uint16_t>(0xFF00u + C.data));
      break;
    case 0xF3:
      DI();
      break;
...
void CPU::addTwoByteValueToRegisterPair(RegisterPair& dest, uint16_t data) {
  ...
  if (((dest.getData() & 0xFFF) + (data & 0xFFF)) & 0x1000) {
    setFlags(H_FLAG);
  } else {
    unsetFlags(H_FLAG);
  }
...
void CPU::subByteValueFromA(uint8_t data) {
  ...
  if ((static_cast<int16_t>(A.data & 0xF) - static_cast<int16_t>(data & 0xF)) <
      0) {
    setFlags(H_FLAG);
  } else {
    unsetFlags(H_FLAG);
...
void CPU::DI() { Globals::IME = false; }

void CPU::EI() { enableInterruptsNextCycle = true; }
...
void CPU::DAA() {
  ...
  if (!checkFlags(N_FLAG)) {
    if (checkFlags(C_FLAG) || (A.data > 0x99)) {
      A.data += 0x60;
      carry = true;
    }
    if (checkFlags(H_FLAG) || ((A.data & 0xF) > 0x9)) {
      A.data += 0x6;
    }
  } else if (checkFlags(C_FLAG)) {
    carry = true;
...
void CPU::LD(Register& dest, Register src) { dest = src; }

void CPU::LD(Register& dest, uint16_t address) {
  dest.data = readMemoryAndIncrementCycles(address);
}

void CPU::LD(RegisterPair address, Register src) {
  writeMemoryAndIncrementCycles(src.data, address.getData());
}

void CPU::LD(Register& dest, uint8_t value) { dest = value; }

void CPU::LD(RegisterPair& dest, uint16_t value) { dest = value; }

void CPU::LD(Register& dest, RegisterPair address) {
  dest = readMemoryAndIncrementCycles(address.getData());
}
...
void CPU::LD(SpecialRegister& dest, uint16_t value) { dest.data = value; }

void CPU::LD(uint16_t address, SpecialRegister src) {
  writeMemoryAndIncrementCycles(src.getLSB(), address);
  writeMemoryAndIncrementCycles(src.getMSB(), address + 1);
}

void CPU::LD(uint16_t address, Register src) {
  writeMemoryAndIncrementCycles(src.data, address);
}

void CPU::LD(uint16_t address, uint8_t data) {
...
void CPU::LDI(Register& src, bool setA) {
  ...
  if (setA) {
    A = readMemoryAndIncrementCycles(HL.getData());
  } else {
    writeMemoryAndIncrementCycles(src.data, HL.getData());
  }
...
void CPU::INC(uint16_t address) {
  uint8_t data = readMemoryAndIncrementCycles(address);
...
void CPU::DEC(uint16_t address) {
  ...
  if ((static_cast<int8_t>(data & 0xF) - 1) < 0) {
    setFlags(H_FLAG);
  } else {
    unsetFlags(H_FLAG);
  }
...
void CPU::ADD(Register src) { addByteValueToA(src.data); }

void CPU::ADD(uint8_t data) { addByteValueToA(data); }

void CPU::ADD(uint16_t address) {
  addByteValueToA(readMemoryAndIncrementCycles(address));
}
...
void CPU::SUB(Register src) { subByteValueFromA(src.data); }

void CPU::SUB(uint16_t address) {
  subByteValueFromA(readMemoryAndIncrementCycles(address));
}

void CPU::SUB(uint8_t data) { subByteValueFromA(data); }

void CPU::AND(Register src) { andByteValueAgainstA(src.data); }

void CPU::AND(uint16_t address) {
  andByteValueAgainstA(readMemoryAndIncrementCycles(address));
}

void CPU::AND(uint8_t data) { andByteValueAgainstA(data); }

void CPU::OR(Register src) { orByteValueAgainstA(src.data); }

void CPU::OR(uint16_t address) {
  orByteValueAgainstA(readMemoryAndIncrementCycles(address));
}

void CPU::OR(uint8_t data) { orByteValueAgainstA(data); }

void CPU::XOR(Register src) { xorByteValueAgainstA(src.data); }

void CPU::XOR(uint16_t address) {
  xorByteValueAgainstA(readMemoryAndIncrementCycles(address));
}

void CPU::XOR(uint8_t data) { xorByteValueAgainstA(data); }

void CPU::CP(Register src) { cpByteValueAgainstA(src.data); }

void CPU::CP(uint16_t address) {
  cpByteValueAgainstA(readMemoryAndIncrementCycles(address));
}

void CPU::CP(uint8_t data) { cpByteValueAgainstA(data); }

void CPU::ADC(Register src) { adcByteValueAgainstA(src.data); }

void CPU::ADC(uint16_t address) {
  adcByteValueAgainstA(readMemoryAndIncrementCycles(address));
}

void CPU::ADC(uint8_t data) { adcByteValueAgainstA(data); }

void CPU::SBC(Register src) { sbcByteValueAgainstA(src.data); }

void CPU::SBC(uint16_t address) {
  sbcByteValueAgainstA(readMemoryAndIncrementCycles(address));
}

void CPU::SBC(uint8_t data) { sbcByteValueAgainstA(data); }

void CPU::POP(RegisterPair& dest) {
  dest.low->data = readMemoryAndIncrementCycles(SP++.data);
  dest.high->data = readMemoryAndIncrementCycles(SP++.data);
...
void CPU::RL(uint16_t address) {
  uint16_t shiftedData = readMemoryAndIncrementCycles(address) << 1;
  ...
  writeMemoryAndIncrementCycles(shiftedData & 0xFF, address);
}
...
void CPU::RES(uint8_t position, Register& src) { src.data &= ~(1 << position); }
...
void CPU::SET(uint8_t position, Register& src) { src.data |= 1 << position; }
