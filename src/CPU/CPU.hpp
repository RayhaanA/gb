...
class CPU {
 private:
  ...
  bool enableInterruptsNextCycle =
      false;  // There's a one cycle delay on enabling IME through EI
              // instruction
  ...
  const std::vector<uint16_t> INTERRUPT_VECTORS = {
      V_BLANK_INTERRUPT_VECTOR, LCDC_INTERRUPT_VECTOR, TIMER_INTERRUPT_VECTOR,
      SERIAL_INTERRUPT_VECTOR, LAST_INTERRUPT_VECTOR};

  const uint8_t TIMER_INTERRUPT_FLAG = 1 << 2;
  ...
  bool haltBugActive =
      false;  // System encounters a bug when halt mode is entered, IME = 0, and
              // an Interrupt is ready and enabled This affects the system by
              // the next instruction not incrementing the PC
  void handleInterrupts();
  bool checkForInterrupts();

  // Increment cycles, check for interrupt enabling, increment timers, check for
  // active DMA, forward time change to PPU
  void incrementCycleCount();

  uint8_t readMemoryAndIncrementCycles(uint16_t address) {
    incrementCycleCount();
    // Can only access HRAM during DMA (todo: actually maybe not according to
    // mooneye)
    // if (dmaActive && !memory->addressInHRAM(address)) {
    //    return UNDEFINED_READ;
    // }
    if (address == Address::JOYPAD_REG_ADDR) {
  ...
  // Bug that happens when doing stuff in 0xFE00 - 0xFEFF range during mode 2 of
  // PPU
  bool oamBugShouldOccur(uint16_t address) {
    return ppu->getMode() == PPUMode::OAM_SEARCH &&
           memory->addressInOAMBugRegion(address) && ppu->getDisplayEnabled();
  }
  void triggerOAMBug(bool read);
  void triggerOAMBugIncrease();
  uint8_t oamBugWrite(uint8_t a, uint8_t b, uint8_t c) {
    return ((a ^ c) & (b ^ c)) ^ c;
  }
  uint8_t oamBugRead(uint8_t a, uint8_t b, uint8_t c) { return b | (a & c); }
  uint8_t oamBugReadIncrease(uint8_t a, uint8_t b, uint8_t c, uint8_t d) {
 ...
 public:
  ...
  uint8_t getIF() { return memory->directRead(Address::IF_REG_ADDR); }
  uint8_t getIE() { return memory->directRead(Address::IE_REG_ADDR); }
  ...
  void runUntilRomStart() {
    if (PC.data >= MMU::START_OF_ROM) {
      throw std::runtime_error(
          "Tried to run boot rom but PC was already inside of main rom!\n");
    }

    while (PC.data != MMU::START_OF_ROM) {
      tick();
    }

    for (size_t i = 0; i < MMU::START_OF_ROM; ++i) {
  ...
  void runOneFrame() {
    while (!frameDone) {
      if (Globals::needToRunBootRom && PC.data >= MMU::START_OF_ROM) {
        for (size_t i = 0; i < MMU::START_OF_ROM; ++i) {
          memory->getMemory()[i] = memory->getBootAreaRemap()[i];
        }
