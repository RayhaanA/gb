...
public:
    ...
    uint8_t readJoypadState() {
        uint8_t currValue = memory->getMemory()[JOYPAD_REG_ADDR];
        currValue ^= 0xFF;
    ...
    void pressKey(sf::Keyboard::Key key) {
        ...
        uint8_t joypad = memory->getMemory()[JOYPAD_REG_ADDR];
        
        bool requestInterrupt = false;
        if ((buttonPressed && !(joypad & SELECT_BUTTONS)) || (!buttonPressed && !(joypad & SELECT_DIRECTION_KEYS))) {
            requestInterrupt = true;
        }

        if (requestInterrupt && !wasUnset) {
            memory->getMemory()[IF_REG_ADDR] |= JOYPAD_INTERRUPT_FLAG & 0x1F;
        }
    }
