...
struct Register {
  ...
  Register() = default;
  Register(uint8_t value) : data(value) {}
  ~Register() {}
  ...
  friend std::ostream& operator<<(std::ostream& os, const Register& r) {
    os << static_cast<uint16_t>(r.data) << " | " << std::hex
       << static_cast<uint16_t>(r.data) << std::dec << " | "
...
struct RegisterPair {
  ...
  RegisterPair(Register* low, Register* high) : low(low), high(high) {}
  ...
  inline uint16_t getData() const { return high->data << 8 | low->data; }
  inline uint8_t getMSB() const { return high->data; }
  inline uint8_t getLSB() const { return low->data; }
  ...
  bool operator==(RegisterPair& other) const { return this == &other; }
...
struct SpecialRegister {
  ...
  SpecialRegister() : data(0) {}
  ~SpecialRegister() = default;
  uint8_t getMSB() const { return (data & 0xFF00) >> 8; }
  uint8_t getLSB() const { return data & 0xFF; }
  ...
  bool operator==(SpecialRegister& other) const { return this == &other; }
