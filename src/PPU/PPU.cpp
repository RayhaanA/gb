  ...
  switch (mode) {
      ...
      if (modeCycles >= H_BLANK_CYCLES) {
        ...
        if (ly == 143) {
          ...
          if (controlRegister.displayEnable) {
            if (Globals::wroteZeroToVBLIF) {
              Globals::wroteZeroToVBLIF = false;
            } else {
    ...
    case PPUMode::V_BLANK:
      if (modeCycles >= V_BLANK_CYCLES) {
        ...
        if (ly == 153) {
          // The OAM condition for STAT IRQ is ignored for the first OAM phase
          // after v-blank
          mode = PPUMode::OAM_SEARCH;
          resetLY();
        } else {
    ...
    case PPUMode::DATA_TRANSFER:
      if (modeCycles < DATA_TRANSFER_CYCLES) {
        ...
        /* if (!controlRegister.bgWindowDisplayPriority && pixelFetchState ==
        PixelFetchState::GET_TILE_ID) { if (bgFifo.size() <= 8) { for (size_t i
        = 0; i < 8; ++i) { bgFifo.push({Color::WHITE, PaletteID::BGP, })
                }
            }
        }
        ++fetcherCycles;
        ...
        case PixelFetchState::GET_TILE_ID:
        {
            if (!controlRegister.bgWindowDisplayPriority) {
                ...
                bool inWindow = controlRegister.windowDisplayEnable && scx >=
        wx;

                if (!inWindow) {
                    tileMapAddress =
        mapStartValues[static_cast<uint8_t>(controlRegister.bgTileMapDisplaySelect)];
                    pixelFetcherX = (pixelFetcherX + scx / 8) & 0x1F;
                    pixelFetcherY = (ly + scy) & 0xFF;
                }
                else {
                    tileMapAddress =
        mapStartValues[static_cast<uint8_t>(controlRegister.windowTileMapSelect)];
        ...
        case PixelFetchState::PUSH:
            ...
            break;
        default:
            std::cout << "Unhandled pixel fetcher state " <<
        static_cast<uint16_t>(pixelFetchState) << std::endl; break;
        }*/
      } else {
        haventDrawnYet = true;
        modeCycles = -Globals::CYCLES_PER_INCREMENT;
...
void PPU::writeRegistersValues(uint16_t address) {
  ...
  switch (address) {
    ...
    case Address::SCY_REG_ADDR:
      scy = (*memory)[address];
      break;
    case Address::SCX_REG_ADDR:
      scx = (*memory)[address];
      break;
    case Address::LY_REG_ADDR:
      ly = (*memory)[address];
      break;
    case Address::LYC_REG_ADDR:
      lyc = (*memory)[address];
      if (lyc == ly) {
        if (statusRegister.lyCoincidenceInterrupt) {
          requestInterrupt(STAT_INTERRUPT_FLAG);
    ...
    case Address::OBP1_REG_ADDR:
      ...
      break;
    case Address::WY_REG_ADDR:
      wy = (*memory)[address];
      break;
    case Address::WX_REG_ADDR:
...
uint8_t PPU::readRegisterValues(uint16_t address) {
  switch (address) {
    case Address::CONTROL_REG_ADDR:
      (*memory)[Address::CONTROL_REG_ADDR] =
          (static_cast<uint8_t>(controlRegister.displayEnable) << 7) |
          (static_cast<uint8_t>(controlRegister.windowTileMapSelect) << 6) |
          (static_cast<uint8_t>(controlRegister.windowDisplayEnable) << 5) |
          (static_cast<uint8_t>(controlRegister.bgAndWindowTileDataSelect)
           << 4) |
      ...
      break;
    case Address::STATUS_REG_ADDR:
      (*memory)[Address::STATUS_REG_ADDR] =
          (static_cast<uint8_t>(statusRegister.lyCoincidenceInterrupt) << 6) |
      ...
      break;
    case Address::SCY_REG_ADDR:
      (*memory)[Address::SCY_REG_ADDR] = scy;
      break;
    case Address::SCX_REG_ADDR:
      (*memory)[Address::SCX_REG_ADDR] = scx;
      break;
    case Address::LY_REG_ADDR:
      (*memory)[Address::LY_REG_ADDR] = ly;
      break;
    case Address::LYC_REG_ADDR:
      (*memory)[Address::LYC_REG_ADDR] = lyc;
      break;
    case Address::BGP_REG_ADDR:
      (*memory)[Address::BGP_REG_ADDR] =
          bgPalette[0].second | bgPalette[1].second << 2 |
          bgPalette[2].second << 4 | bgPalette[3].second << 6;
      break;
    case Address::OBP0_REG_ADDR:
      (*memory)[Address::OBP0_REG_ADDR] = ob0Palette[1].second << 2 |
                                          ob0Palette[2].second << 4 |
                                          ob0Palette[3].second << 6;
      break;
    case Address::OBP1_REG_ADDR:
      (*memory)[Address::OBP1_REG_ADDR] = ob1Palette[1].second << 2 |
                                          ob1Palette[2].second << 4 |
                                          ob1Palette[3].second << 6;
      break;
    case Address::WY_REG_ADDR:
      (*memory)[Address::WY_REG_ADDR] = wy;
      break;
    case Address::WX_REG_ADDR:
      (*memory)[Address::WX_REG_ADDR] = wx + WX_OFFSET;
      break;
    default:
      break;
  }
...
// Naive method to draw scan line until I get around to implementing pixel
// fetcher (todo) Pushes entire scanline at once at end of data transfer phase
void PPU::drawScanLine() {
  ...
  uint16_t tileDataAddr = dataStartValues[static_cast<uint8_t>(
      controlRegister.bgAndWindowTileDataSelect)];
  bool unsignedOffset = controlRegister.bgAndWindowTileDataSelect;
  uint16_t tileMapAddr = 0;

  if (!inWindow) {
    tileMapAddr = mapStartValues[static_cast<uint8_t>(
        controlRegister.bgTileMapDisplaySelect)];
  } else {
    tileMapAddr = mapStartValues[static_cast<uint8_t>(
        controlRegister.windowTileMapSelect)];
  }
  ...
  for (uint8_t i = 0; i < Globals::WIDTH; ++i) {
    ...
    int16_t tileNum = unsignedOffset ? (*memory)[tileAddr]
                                     : static_cast<int8_t>((*memory)[tileAddr]);

    uint16_t tileLocation =
        tileDataAddr +
        (unsignedOffset ? (tileNum * 16) : ((tileNum + 128) * 16));
...
void PPU::drawSprites() {
  ...
  for (auto& sprite : spritesForCurrLine) {
    ...
    for (int8_t i = 7; i >= 0; --i) {
      ...
      sf::Color pixelColor = sprite.paletteNumber() ? ob1Palette[color].first
                                                    : ob0Palette[color].first;
