...
disassemble(std::vector<uint8_t>& rom) {
  ...
  for (int32_t i = 0; i < rom.size(); ++i) {
    if (i >= 0xA8 && i < 0xE0 || i >= 0x104 && i <= 0x14F ||
        i >= 0x8000 && i <= 0x9FFF) {
      std::string ss =
          printHexToString(i, 4) + ": db " + printHexToString(rom[i], 2);
      lines.push_back(ss);
      continue;
    }
    std::string ss;
    ...
    if (currOpVal == 0xCB) {
      ...
      if (currOpVal == 0x08) {
        ...
        if (currOpVal == 0x18 || currOpVal == 0x20 || currOpVal == 0x28 ||
            currOpVal == 0x30 || currOpVal == 0x38 || currOpVal == 0xE8) {
          int8_t value = static_cast<int8_t>(rom[++i]);
          ss += " ";
